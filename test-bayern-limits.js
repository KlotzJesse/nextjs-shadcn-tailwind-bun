...
async function testBayernSelection() {
  try {
    ...
    console.log(`   📍 Bayern coordinates: [${bayernResult.coordinates[0]}, ${bayernResult.coordinates[1]}]`);
    
    // Step 2: Get boundary postal codes
    console.log("2️⃣ Getting postal codes within Bayern boundary...");
    const boundaryResponse = await fetch("http://localhost:3000/api/postal-codes/search-by-boundary", {
      method: "POST", 
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        areaName: "Bayern",
    ...
    console.log(`   📮 First few codes: ${boundaryData.postalCodes.slice(0, 5).join(", ")}...`);
    console.log(`   📮 Last few codes: ...${boundaryData.postalCodes.slice(-5).join(", ")}`);
    
    // Step 3: Verify limits
    if (boundaryData.count >= 2000) {
      console.log("   ✅ LIMIT FIX SUCCESSFUL: Got more than 2000 postal codes");
    } else {
      console.log("   ❌ LIMIT ISSUE: Only got", boundaryData.count, "postal codes");
    }
...
testBayernSelection().then(result => {
  ...
  console.log(`   4. All ${result.boundary?.count || 'X'} postal codes in Bavaria get selected`);
