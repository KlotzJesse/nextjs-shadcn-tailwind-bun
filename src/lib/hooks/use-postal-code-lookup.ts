...
export function usePostalCodeLookup({
  ...
  const [lastLookupResult, setLastLookupResult] = useState<PostalCodeLookupResult | null>(null);
  ...
  }, [data, onPostalCodeFound]);

  const clearLastLookup = useCallback(() => {
    setLastLookupResult(null);
  }, []);

  // Helper function to find postal code by coordinates
  const findPostalCodeByCoords = useCallback((lng: number, lat: number) => {
    ...
    for (const feature of data.features) {
      if (feature.geometry.type === "Polygon") {
        ...
        if (isPointInPolygon([lng, lat], polygon as number[][])) {
          return feature.properties?.code || feature.properties?.PLZ || feature.properties?.plz;
        }
