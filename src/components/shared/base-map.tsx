...
export function BaseMap({
    ...
    (polygon: number[][]): string[] => {
      ...
      data.features.forEach((feature: any) => {
        ...
        if (centroid === undefined) {
          const centroidResult = getLargestPolygonCentroid(
            feature as Feature<Polygon | MultiPolygon, GeoJsonProperties>
          );
          centroid =
            centroidResult && centroidResult.length >= 2
              ? [centroidResult[0], centroidResult[1]]
              : null;
        ...
        if (isPointInPolygon(centroid, polygon as [number, number][])) {
  ...
  const findFeaturesInCircle = useCallback(
    (center: [number, number], radiusDegrees: number): string[] => {
      ...
      data.features.forEach((feature: any) => {
        ...
        const dLat = Math.abs(lat2 - centerY);
        const dLng = Math.abs(lng2 - centerX);
  ...
  const handleTerraDrawSelection = useCallback(
    (featureIds: (string | number)[]) => {
      console.log(
        "handleTerraDrawSelection called with feature IDs:",
        featureIds
      );
    ...
    import("maplibre-gl")
      .then((maplibregl) => {
        ...
        map.current.on("style.load", () => {
          if (isMounted) setStyleLoaded(true);
        });
      })
      .catch((error) => {
  ...
  }, [center, zoom, data]); // Include data in dependencies

  // Create or update layers when data changes
  useEffect(() => {
    // Add validation for data before proceeding
    if (
      !map.current ||
      !isMapLoaded ||
      !styleLoaded ||
    ...
    if (statesData) {
      ...
      if (!map.current.getSource("state-boundaries-label-points")) {
        map.current.addSource("state-boundaries-label-points", {
          type: "geojson",
          data: makeLabelPoints(
            statesData as FeatureCollection<Polygon | MultiPolygon>,
            "name"
          ),
        });
      } else {
        map.current
          .getSource("state-boundaries-label-points")
          .setData(
            makeLabelPoints(
              statesData as FeatureCollection<Polygon | MultiPolygon>,
              "name"
  ...
  // If data is not valid, show loading skeleton
  if (
    !data ||
    !data.features ||
    !Array.isArray(data.features) ||
        ...
        <div className="relative w-full h-full">
          ...
          {isDrawingToolsVisible && (
            ...
            >
              <ErrorBoundary>
                <Suspense
                  fallback={<Skeleton className="w-56 h-80 rounded-lg" />}
                >
                  <DrawingTools
